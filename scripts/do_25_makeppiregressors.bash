...
set subj = $1
set PPI_res = ${topdir}/${subj}/ACC_PPI_Incongruent_results
set PPI_input = ${topdir}/${subj}/ACC_PPI_Incongruent                                                              \
set Stimuli_dir = /Volumes/NNU_Neurofeedback/derivatives/BOLDanalysis/${subj}/Visit1/GAM/stimuli
set stim_files = ( ${Stimuli_dir}/*.txt)
set stim_labs =  ( First_Congruent First_Incongruent Last_Congruent Last_Incongruent )
set stim_dur  = ( 1 1 1 1 )       # durations, in seconds 
set seed = $PPI_input/ppi.leftACC${subj}.1D
set plabel = ACC
set basis = GAM                                                                      \


set NT = ( 138 138 138 138 )        # num time points per run
set TR = 3
set TRnup = 30                  # oversample rate
...
set run_lens = (414 414 414 414)  # run lengths, in seconds
...
foreach rind ( `count -digits 1 1 $#NT` )
         3dTfitter                                                           \
             -RHS      $pprev.seed.$TRnup.r$rind.1D                          \
...
foreach rind ( `count -digits 1 1 $#NT` )
   set neuro_seed = $neuro_prefix.neuro.r$rind.1D
     waver                                                                   \
         -FILE    $TRup $hrf_file                                            \
         -input   $neuro_seed                                                \
         -numout  $nt > $prefix.c.seed.r$rind.reBOLD.1D
end
...
echo                                                                         \
    ...
    seeds:                                                                   \
    $seed_up                                                                 \
    $prefix.d.{seed,task}.rall.reBOLD.1D
